...
#& FilePicker function for selecting input file via explorer window or via fzf or text on macOS / Linux
function Get-FilePicker {
  ...
  if ($IsWindows) {
    try {
      ...
      $ofd.Title            = "Select file for $Script:ScriptName"
      ...
      $filterMap = @{
        ...
        "ps"    = "PowerShell files (*.ps1;*.psm1;*.psd1)|*.ps1;*.psm1;*.psd1"
        "bicep" = "Bicep files (*.bicep)|*.bicep"
        "all"   = "All files (*.*)|*.*"
      }

      $ofd.Filter = ($Filter | ForEach-Object { $filterMap[$_] }) -join "|"
      $ofd.FilterIndex = $Filter.Count
      ...
      Write-Warning "GUI file picker not available. Falling back to CLI mode."
    }
  }

  # CLI fallback for macOS/Linux (and Windows if GUI fails)
  $patternList = @()
  foreach ($f in $Filter) {
    switch ($f) {
      "txt"   { $patternList += "*.txt" }
      "csv"   { $patternList += "*.csv" }
      "json"  { $patternList += "*.json" }
      "xml"   { $patternList += "*.xml" }
      "ps"    { $patternList += "*.ps1","*.psm1","*.psd1" }
      "bicep" { $patternList += "*.bicep" }
      "all"   { $patternList += "*.*" }
    }
  }

  $files = foreach ($pattern in $patternList) {
    Get-ChildItem -Path $Path -File -Filter $pattern | Select-Object -ExpandProperty FullName
  }
  ...
  if ($fzfExists) {
    ...
    if ($selected) {
      ...
      Write-Host "No file selected via fzf"
      return $null
    }
  }
  else {
    ...
    $choice = Read-Host "Enter the number of the file you want"
